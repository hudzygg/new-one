...
app.get('/api/wallet/summary', async (req, res) => {
	try {
		...
		if (!isValidEthAddress(address)) return res.status(400).json({ error: 'Invalid address' });
		const data = await fetchPerformanceSummary(address);
...
app.get('/api/token/alpha', async (req, res) => {
	try {
		const token = String(req.query.token || '').toLowerCase();
		const offset = Number(req.query.offset || 0);
		if (!isValidEthAddress(token)) return res.status(400).json({ error: 'Invalid token address' });
